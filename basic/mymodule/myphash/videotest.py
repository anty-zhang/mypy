...
print 'libphash_path is: ', libphash_path
...
class MyHash(ctypes.Structure):
    _fields_ = [
        ('hashid', ctypes.c_ulonglong),
    ]


if __name__ == '__main__':
    video_name = '/home/andy/xx/Videos/sample/xx.mp4'
    ph_dct_videohash = libphash.ph_dct_videohash
    ph_dct_videohash.restype = ctypes.Array(ctypes.POINTER(ctypes.c_int))
    # ph_dct_videohash.restype = [ctypes.POINTER(ctypes.c_ulonglong)]
    ph_dct_videohash.argtypes = [ctypes.c_char_p, ctypes.POINTER(ctypes.c_int)]

    l = ctypes.c_int()
    # tt = ctypes.POINTER(ctypes.c_ulonglong)
    print 'l=', l
    # tt = ctypes.POINTER(ctypes.c_ulonglong)
    tt = ph_dct_videohash(video_name, byref(l))
    # print 'tt=', tt.contents.value
    print 'tt=', tt, ' ', tt.contents
    print 'l=', l.value
