...
if __name__ == "__main__":
    ...
    documentNode = xmlGenerator.createNode("DOCUMENT")
    xmlGenerator.setNodeAttr(documentNode, "content_method", "full")
    xmlGenerator.addNode(node=documentNode)
    # item
    itemNode = xmlGenerator.createNode("item")
    xmlGenerator.addNode(itemNode, documentNode)


    # key node
    keyNode = xmlGenerator.createNode("key")
    xmlGenerator.setNodeValue(keyNode, u"百思不得姐")
    xmlGenerator.addNode(keyNode, itemNode)

    # display node
    displayNode = xmlGenerator.createNode("display")
    xmlGenerator.addNode(displayNode, itemNode)

    # title node
    titleNode = xmlGenerator.createNode("title")
    xmlGenerator.setNodeValue(titleNode, u"李克强：科学编制“十三五”《规划纲要》")
    xmlGenerator.addNode(titleNode, displayNode)

    # url node
    urlNode = xmlGenerator.createNode("url")
    xmlGenerator.setNodeValue(urlNode, u'http://www.ce.cn/xwzx/gnsz/szyw/201511/17/t20151117_7040754.shtml')
    xmlGenerator.addNode(urlNode, displayNode)
